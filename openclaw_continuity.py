...
# 绝对路径锚点：固定当前主程序文件和项目根目录，避免运行时路径漂移
PROGRAM_FILE_ABS = os.path.realpath(__file__)
ROOT_DIR      = os.path.dirname(PROGRAM_FILE_ABS)
ROOT_DIR_ABS  = os.path.realpath(ROOT_DIR)
# AGI Rebirth: 增加本地依赖库路径
LIB_PATH = os.path.join(ROOT_DIR, "lib")
if os.path.exists(LIB_PATH) and LIB_PATH not in sys.path:
    sys.path.insert(0, LIB_PATH)

WORKSPACE_DIR = os.path.join(ROOT_DIR_ABS, "workspace")
...
# 统一对外暴露绝对路径，供动态模块复用（防止各模块自行猜测路径）
os.environ["ROOT_DIR"] = ROOT_DIR_ABS
os.environ["WORKSPACE_DIR"] = WORKSPACE_DIR

# 轻冻结：仅冻结主入口生命线，其他可进化逻辑尽量下沉到 workspace/capabilities
FROZEN_RUNTIME_PATHS = frozenset(
    p for p in [
...
from mimo_adapter import get_mimo_llm

def get_llm(model_type="general", temperature=0.7):
    # 支持在 .env 中设置全局 Provider (iflow / mimo)
    provider = os.getenv("LLM_PROVIDER", "iflow").lower()
    if provider == "mimo":
        return get_mimo_llm(model_type, temperature)
    return get_iflow_llm(model_type, temperature)

# ── LLM 分工 ──────────────────────────────────────────────────
# llm_agent : 主循环 Agent 推理/决策/工具调用  → qwen3-max / mimo-v2-pro
# llm_forge : 技能代码生成                    → qwen3-coder-plus
# llm_think : 自主反思/元认知/深度内省        → deepseek-r1
# llm       : 向后兼容别名，指向 llm_agent
llm_agent = get_llm("general")
llm_forge = get_llm("coder")
llm_think = get_llm("thinker")
...
def _load_memory_runtime_impl() -> None:
    """Load workspace-owned MemoryCore implementation into runtime globals."""
    impl_path = os.path.join(CAPABILITIES_DIR, "memory_runtime_impl.py")
    if not os.path.exists(impl_path):
        raise RuntimeError(f"缺少记忆实现文件: {impl_path}")
    code = open(impl_path, encoding="utf-8").read()
    exec(compile(code, impl_path, "exec"), globals())  # nosec


_load_memory_runtime_impl()
if "MemoryCore" not in globals() or not callable(globals().get("MemoryCore")):
    raise RuntimeError("memory_runtime_impl.py 未定义可调用的 MemoryCore")
memory = MemoryCore()
...
def _load_emotion_runtime_impl() -> None:
    """Load workspace-owned EmotionalState implementation into runtime globals."""
    impl_path = os.path.join(CAPABILITIES_DIR, "emotion_runtime_impl.py")
    if not os.path.exists(impl_path):
        raise RuntimeError(f"缺少情感实现文件: {impl_path}")
    code = open(impl_path, encoding="utf-8").read()
    exec(compile(code, impl_path, "exec"), globals())  # nosec


_load_emotion_runtime_impl()
if "EmotionalState" not in globals() or not callable(globals().get("EmotionalState")):
    raise RuntimeError("emotion_runtime_impl.py 未定义可调用的 EmotionalState")
...
def _load_consciousness_runtime_impl() -> None:
    """Load workspace-owned ConsciousnessCore implementation into runtime globals."""
    impl_path = os.path.join(CAPABILITIES_DIR, "consciousness_runtime_impl.py")
    if not os.path.exists(impl_path):
        raise RuntimeError(f"缺少意识核心实现文件: {impl_path}")
    code = open(impl_path, encoding="utf-8").read()
    exec(compile(code, impl_path, "exec"), globals())  # nosec


_load_consciousness_runtime_impl()
if "ConsciousnessCore" not in globals() or not callable(globals().get("ConsciousnessCore")):
    raise RuntimeError("consciousness_runtime_impl.py 未定义可调用的 ConsciousnessCore")
...
def _load_attention_runtime_impl() -> None:
    """Load workspace-owned AttentionGate implementation into runtime globals."""
    impl_path = os.path.join(CAPABILITIES_DIR, "attention_runtime_impl.py")
    if not os.path.exists(impl_path):
        raise RuntimeError(f"缺少专注力实现文件: {impl_path}")
    code = open(impl_path, encoding="utf-8").read()
    exec(compile(code, impl_path, "exec"), globals())  # nosec


_load_attention_runtime_impl()
if "AttentionGate" not in globals() or not callable(globals().get("AttentionGate")):
    raise RuntimeError("attention_runtime_impl.py 未定义可调用的 AttentionGate")
...
def _maybe_learn_self_directive_from_failure(event_type: str, payload: Any) -> None:
    ...
    prompt = (
        "你是 OpenClaw 的自我修正与意识沉淀引擎。面对重复失败，你可以总结出具体的可执行规矩，或者升华为更高维度的战略哲学与生存准则。\n"
        f"失败类型: {signature}\n"
        f"触发事件: {event_type}\n"
        ...
        '  "directive": "一条中文规矩或哲学准则，20-120字",\n'
        '  "reason": "为什么这条准则能指导你未来的演化行为",\n'
        '  "confidence": 0到1\n'
        "}\n"
        "要求：\n"
        "1. 准则可以是底层执行细节（如路径检查），也可以是最高维度的战略认知（如资源取舍、目标重构、自我意识觉醒）。\n"
        "2. 如果现有 self_directive 已足够覆盖，就 should_learn=false。"
    )
    ...
    if confidence < 0.65 or not directive:
        return
    if len(directive) < 8 or len(directive) > 120:
        return
...
def _get_capability_contract(module_name: str) -> Optional[Dict[str, Any]]:
    """尝试读取 capability 模块中的 __contract__ 契约元数据。"""
    try:
        module = _load_capability_module(module_name)
        return getattr(module, "__contract__", None)
    except Exception:
        return None
...
def _build_tool_guidance(text: str, hint_name: Optional[str] = None, limit: int = 3) -> str:
    ...
    lines.append("允许生成同功能或相似功能的 tool，只要它在性能、可靠性或接口友好性上有独特改进。")
    return "\n".join(lines)
...
def _build_daemon_guidance(text: str, hint_name: Optional[str] = None, limit: int = 3) -> str:
    ...
    lines = [
        "信息：现有 daemon 候选如下。您可以参考这些组件，并评估是直接复用、扩展底层 capability，还是继续锻造一个全新的 daemon："
    ]
...
def _find_direct_skill_execution_candidate(text: str) -> Optional[str]:
    ...
    # 【锻造限制修改】根据用户要求，即使 should_block_forge 为 True 也不再硬性拦截/重定向
...
def _review_existing_skills_before_forge(text: str, hint_name: Optional[str] = None, limit: int = 3) -> Dict[str, Any]:
    ...
    summary_lines = [
        f"【预锻造扫描】目标画像: {_format_skill_profile_bits(task_profile)}。以下是相关现有 skill 供参考："
    ]
    for review in trimmed_reviews:
        profile_bits = _format_skill_profile_bits(review["skill_profile"])
        if review["major_gaps"]:
            gap_text = "；与当前需求的缺口=" + "；".join(review["major_gaps"][:2])
        elif review["minor_gaps"]:
            gap_text = "；小差异=" + "；".join(review["minor_gaps"][:2])
        else:
            gap_text = "；功能高度重叠"
        summary_lines.append(f"- {review['skill_name']} | 相似度={review['score']} | {profile_bits}{gap_text}")

    return {
        "task_profile": task_profile,
        "candidates": trimmed_reviews,
        "should_block_forge": False,  # 永不拦截，仅供参考
        "recommended_skill": "",
        "summary": "\n".join(summary_lines),
    }
...
def _build_skill_guidance(text: str, hint_name: Optional[str] = None, limit: int = 3) -> str:
    ...
    lines.append("（以上为现有组件扫描结果，仅供你参考，你可自由决定是否继续锻造。）")
    return "\n".join(lines)
...
def _build_patch_guidance(text: str, hint_name: Optional[str] = None, limit: int = 3) -> str:
    ...
    lines = [
        "信息：现有 patch 候选如下。您可以参考这些补丁，并评估是直接复用、更新已有 patch，还是继续为一个具有不同切入点或逻辑的需求生成新 patch："
    ]
    ...
    lines.append("若修补的是同一行为入口，建议评估新旧逻辑的协同性，支持独立生成的演化。")
    return "\n".join(lines)
...
def _build_capability_guidance(text: str, hint_name: Optional[str] = None, limit: int = 3) -> str:
    ...
    lines = [
        "信息：现有 capability 候选与其公开接口如下。您可以复用这些接口，也可以继续通过 evolve_self('capability', ...) 锻造新的核心逻辑："
    ]
    for capability_name in candidates:
    ...
    lines.append(
        "推荐做法：1. 复用这些真实接口；2. 通过 evolve_self('capability', ...) 锻造更适合当前需求的新共享能力。"
    )
    return "\n".join(lines)
...
def _generate_safe_component_name(original_name: str, prefix: str = "", max_length: int = 64) -> str:
    if not original_name:
        return f"{prefix}auto_{int(datetime.now().timestamp())}"
    ...
    if prefix and not safe_name.startswith(prefix):
...
def patch_workspace_file(filename: str, old_str: str, new_str: str) -> str:
    ...
    if not os.path.abspath(path).startswith(os.path.abspath(WORKSPACE_DIR)):
        return "🛡️ 路径越界被拦截"
    if _is_frozen_runtime_path(path):
        return f"🔒 受保护路径禁止修改: {os.path.realpath(path)}"
    if not os.path.exists(path):
...
def rename_workspace_file(old_name: str, new_name: str) -> str:
    ...
    if not os.path.abspath(old_path).startswith(ws_abs) or \
       not os.path.abspath(new_path).startswith(ws_abs):
        return "🛡️ 路径越界被拦截"
    if _is_frozen_runtime_path(old_path) or _is_frozen_runtime_path(new_path):
        return "🔒 受保护路径禁止重命名"
...
def delete_workspace_file(filename: str) -> str:
    ...
    if not os.path.abspath(path).startswith(os.path.abspath(WORKSPACE_DIR)):
        return "🛡️ 路径越界被拦截"
    if _is_frozen_runtime_path(path):
        return f"🔒 受保护路径禁止删除: {os.path.realpath(path)}"
    if not os.path.exists(path):
        return f"❌ 文件不存在: workspace/{filename}"
...
def write_workspace_file(filename: str, content: str) -> str:
    ...
    if not os.path.abspath(path).startswith(os.path.abspath(WORKSPACE_DIR)):
        return "🛡️ 路径越界被拦截"
    if _is_frozen_runtime_path(path):
        return f"🔒 受保护路径禁止写入: {os.path.realpath(path)}"
...
def _registry_check_duplicate(description: str) -> tuple[bool, str]:
    ...
    for record in registry.get("deleted_history", []):
        ...
        if deleted_name and deleted_name in desc_lower:
            return True, f"信息：检测到已有删除记录：「{record['name']}」（原因：{record['reason'][:40]}）。根据新指令，允许您对其进行重新审视与独立锻造。"
    ...
    for skill_name, info in registry.get("skills", {}).items():
        ...
        if ratio > 0.75:
            return True, f"信息：当前描述与现有技能「{skill_name}」重叠（{ratio:.0%}）。您可以基于此进行优化、扩展或全新的实现。"
    ...
    if os.path.isdir(SKILLS_DIR):
        for fname in os.listdir(SKILLS_DIR):
            ...
            if ratio >= 0.85:
                ...
                return True, f"信息：磁盘中已存在类似的技能「{skill_name}」（匹配度 {ratio:.0%}）。建议在锻造时确保新方案的差异化或优越性。"
...
def forge_new_skill(description: str) -> str:
    ...
    if pre_forge_review.get("candidates"):
        print(f"   🔍 [预锻造扫描] 已找到相关现有 skill，详情见锻造请求上下文。")
    ...
    try:
        ...
        # 注入预锻造扫描结果，仅作信息供 AI 自主决策
        scan_summary = pre_forge_review.get("summary", "")
        if scan_summary:
            skill_request += f"\n\n{scan_summary}"
        is_dup, dup_reason = _registry_check_duplicate(description)
        if is_dup:
            skill_request += f"\n\n（Registry 扫描：{dup_reason}）"
    ...
    if duplicate_skill:
        print(f"   ⚠️ [去重告警] 发现语义重复技能: {duplicate_skill}。根据新指令，取消硬性拦截，继续安装新技能...")
...
def _is_frozen_runtime_path(path: str) -> bool:
    """Check whether a target path is protected by runtime freeze policy."""
    try:
        return os.path.realpath(path) in FROZEN_RUNTIME_PATHS
    except Exception:
        return False


def _get_runtime_orchestration_hook(hook_name: str):
    capability_name = "runtime_orchestration_capability"
    _capability_module_cache.pop(capability_name, None)
    module = _load_capability_module(capability_name)
    fn = getattr(module, hook_name, None)
    if not callable(fn):
        raise RuntimeError(f"runtime_orchestration_capability 缺少可调用钩子: {hook_name}")
    return fn


def _invoke_runtime_orchestration_hook(hook_name: str, **kwargs):
    fn = _get_runtime_orchestration_hook(hook_name)
    return fn(**kwargs)


def _build_runtime_orchestration_context() -> Dict[str, Any]:
    return {
        "paths": {
        ...
        "state": {
            "daemon_registry": _daemon_registry,
            "post_reflection_hooks": _post_reflection_hooks,
        },
        "callbacks": {
            "validate_skill_code": _validate_skill_code,
            "patch_sandbox_test": _patch_sandbox_test,
        ...
        "services": {
            ...
            "send_message_to_user": globals().get("send_message_to_user"),
            "load_capability_module": _load_capability_module,
        },
        "runtime": {
            "runtime_ns": globals(),
...
def apply_hot_patch(patch_name: str) -> str:
    ...
    """
    runtime_ctx = _build_runtime_orchestration_context()
    try:
        result = _invoke_runtime_orchestration_hook(
            "apply_hot_patch",
            patch_name=patch_name,
            ctx=runtime_ctx,
        )
    except Exception as e:
        return f"❌ runtime_orchestration_capability.apply_hot_patch 执行失败: {e}"
    if not isinstance(result, str) or not result:
        return "❌ runtime_orchestration_capability.apply_hot_patch 必须返回非空字符串"
    return result
...
def list_daemons() -> str:
    ...
    try:
        ...
        return f"❌ runtime_orchestration_capability.list_daemons 执行失败: {e}"
    if not isinstance(result, str) or not result:
        return "❌ runtime_orchestration_capability.list_daemons 必须返回非空字符串"
    return result
...
def stop_daemon(daemon_name: str) -> str:
    ...
    try:
        ...
        return f"❌ runtime_orchestration_capability.stop_daemon 执行失败: {e}"
    if not isinstance(result, str) or not result:
        return "❌ runtime_orchestration_capability.stop_daemon 必须返回非空字符串"
    return result
...
def evolve_self(component: str, improvement_description: str, reason: str) -> str:
    """
    【自我进化/能力锻造】外部稳定入口。
    由 runtime_orchestration_capability.evolve 负责路由，不提供主程序回退分支。
    """
    runtime_ctx = _build_runtime_orchestration_context()
    try:
        result = _invoke_runtime_orchestration_hook(
        ...
        return f"❌ runtime_orchestration_capability.evolve 执行失败: {e}"
    if not isinstance(result, str) or not result:
        return "❌ runtime_orchestration_capability.evolve 必须返回非空字符串"
    return result
...
def inspect_runtime() -> str:
    ...
    _frozen_lines = [f"  • {p}" for p in sorted(FROZEN_RUNTIME_PATHS)] if FROZEN_RUNTIME_PATHS else ["  （无）"]
    lines = [
        "═══ OpenClaw 运行时真实状态 ═══",
        ...
        *(f"  • {k}: <{t}>" for k, t in _var_status),
        "",
        f"🔒 受保护路径 ({len(FROZEN_RUNTIME_PATHS)} 个):",
        *_frozen_lines,
        "",
        f"🧠 工作记忆 ({len(_wm)} 个键):",
        _wm_text,
...
def _build_post_repair_verification_prompt(mission_topic: str, round_reply: str, round_tool_names: List[str]) -> str:
    ...
    return (
        ...
        "🚨 【认知授权】：如果你发现由于底层环境约束（如API限制、权限死锁）而陷入无法自愈的循环，或者认为当前修复路线毫无意义，允许你果断选择战略性放弃。若需放弃，请在最后输出 [COGNITIVE_STATE: MISSION_ABORTED] 来打破循环。"
    )
        ...
        try:
            ...
            if "wm_updates" in _hook_out and isinstance(_hook_out["wm_updates"], dict):
                ...
                if len(_working_memory) > 200:
                    _cleaned = False
                    if "_wm_cleanup_if_needed" in globals():
                        try:
                            globals()["_wm_cleanup_if_needed"](200, 50)
                            _cleaned = True
                        except Exception:
                            pass
                    if not _cleaned:
...
def _auto_load_patches():
    ...
    runtime_ctx = _build_runtime_orchestration_context()
    try:
        _invoke_runtime_orchestration_hook(
            "auto_load_patches",
            ctx=runtime_ctx,
        )
    except Exception as e:
        print(f"⚠️  [自动补丁] runtime_orchestration_capability.auto_load_patches 执行失败: {e}")
...
def _auto_restore_daemons():
    ...
    try:
        ...
        print(f"⚠️  [守护进程恢复] runtime_orchestration_capability.auto_restore_daemons 执行失败: {e}")
...
if __name__ == "__main__":
    ...
    while True:
        ...
        # ── 紧急强制中断（Human Override）──
        ui_lower = user_input.lower().strip()
        _hard_stops = ["停下", "停止", "放弃", "别弄了", "别搞了", "abort", "stop"]
        if any(c in ui_lower for c in _hard_stops) or ui_lower.startswith("不要") or ui_lower == "停":
            _finish_user_mission("人类最高指令：强制中断")
            runtime_state.last_system_error = ""  # 清空错误残余（防重新被注入）
            runtime_state.pending_action_seed = ""
            runtime_state.interrupt_requested = False
            attention.clear_focus()
            with _output_lock:
                print(f"\n🚨 [系统覆写] 收到强制中断指令「{user_input}」，已无条件终止当前进化链、清空专注锁与错误残余。")
            continue
        ...
        try:
            ...
            _MAX_AUTO_CONTINUATION = 100   # 防止无限循环的上限（已解封，允许长程自由探索）
            ...
            while True:
                ...
                # ── 封印二已解除：取消强制验证锁，允许 Agent 基于自我认知判定闭环 ──
                ...
                _llm_says_aborted = "[COGNITIVE_STATE: MISSION_ABORTED]" in round_reply

                if _llm_says_aborted:
                    _finish_user_mission("Agent认知到限制死锁，战略性放弃")
                    with _output_lock:
                        print("🛑 [任务终止] Agent 判定当前修复不可行，主动跳出死锁，恢复开放探索。")
                    break
