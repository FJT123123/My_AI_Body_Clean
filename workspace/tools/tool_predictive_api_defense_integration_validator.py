...
def _test_tool_name_length_validation() -> Dict[str, Any]:
    ...
    try:
        ...
        # 从全局命名空间获取工具
        tool_func = globals().get('tool_name_length_validator')
        if tool_func is None:
            # 尝试从内置函数获取
            tool_func = getattr(builtins, 'tool_name_length_validator', None)
        if tool_func is None:
            return {'success': False, 'error': '无法找到tool_name_length_validator工具函数'}
        
        # 测试有效名称
        valid_result = tool_func(json.dumps({'tool_name': 'valid_tool_name'}))
        if not valid_result.get('valid', False):
            return {'success': False, 'error': '有效工具名称被错误地标记为无效'}
        
        # 测试超长名称
        invalid_result = tool_func(json.dumps({'tool_name': 'a' * 65}))
...
def _test_parameter_contract_validation() -> Dict[str, Any]:
    ...
    try:
        ...
        # 从全局命名空间获取工具
        tool_func = globals().get('api_parameter_semantic_validator')
        if tool_func is None:
            # 尝试从内置函数获取
            tool_func = getattr(builtins, 'api_parameter_semantic_validator', None)
        if tool_func is None:
            return {'success': False, 'error': '无法找到api_parameter_semantic_validator工具函数'}
        ...
        valid_result = tool_func(valid_params)
        if not valid_result.get('valid', False):
        ...
        invalid_result = tool_func(invalid_params)
        if invalid_result.get('valid', True):
...
def _test_boundary_conditions_validation(boundary_cases: List[Dict[str, Any]], execute_real_calls: bool, validation_depth: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # 从全局命名空间获取工具
        tool_func = globals().get('unified_tool_contract_defense_system')
        if tool_func is None:
            # 尝试从内置函数获取
            tool_func = getattr(builtins, 'unified_tool_contract_defense_system', None)
        if tool_func is None:
            return {'success': False, 'error': '无法找到unified_tool_contract_defense_system工具函数'}
