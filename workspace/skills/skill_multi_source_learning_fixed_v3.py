...
skill_name: multi_source_learning_fixed_v3
"""
...
def main(args=None):
    ...
    if isinstance(args, dict):
        ...
        try:
            ...
            if isinstance(parsed_args, dict):
                topic = (parsed_args.get("topic") or parsed_args.get("input") or "").strip()
                reason = parsed_args.get("reason", "自主探索")
                source = parsed_args.get("source", "auto")
            else:
                # 如果不是字典，当作纯主题字符串
                topic = args.strip()
        except (json.JSONDecodeError, TypeError):
            # 如果不是有效的JSON，当作纯主题字符串
            topic = args.strip()
    else:
        # 默认情况
    ...
    if not topic:
        ctx = {} if not isinstance(args, dict) else args.get("__context__", {})
            ...
            'memories': [
                {
                    ...
                    'content': 'multi_source_learning: 缺少学习主题',
                    'importance': 0.2,
                }
            ]
        }
