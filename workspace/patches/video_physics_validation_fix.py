...
def video_physics_regression_validation_fixed(video_path=None, output_dir="./regression_validation", test_patterns=None):
    ...
    if video_path and os.path.exists(video_path):
        ...
        try:
            ...
            generate_result = run_skill("create_high_framerate_test_video", generate_params)
            
            if 'error' in str(generate_result):
                # 如果高帧率视频生成失败，尝试4K压力测试视频
                generate_result = run_skill("create_4k_stress_test_video", "")
    ...
    # 步骤2: 验证原始视频的物理约束 - 使用run_skill来调用工具
    try:
        original_validation_result = run_skill("video_frame_motion_semantic_validator", 
                                             json.dumps({"video_path": original_video_path, 
    ...
    # 步骤4: 验证修复后视频的物理约束
    try:
        repaired_validation_result = run_skill("video_frame_motion_semantic_validator", 
                                             json.dumps({"video_path": repaired_video_path, 
